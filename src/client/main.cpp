...
unordered_map<string, string> commandMap = {
    ...
    {"removefriend", "删除好友，格式removefriend:friendid"},
...
int main(int argc, char **argv){
    ...
    while(true){
        ...
        case 2: // register业务
        {
            ...
            sem_wait(&rwsem);   // 等待信号量，由子线程处理完注册的响应消息后通知这里
        }
        break;
        case 3: // quit业务
            close(clientfd);
...
void removefriend(int clientfd, string str){
    ...
    int len = send(clientfd, buffer.c_str(), strlen(buffer.c_str()) + 1, 0);
    if(-1 == len){
        cerr << "send removefriend msg error -> " << buffer << endl;
    }
}
